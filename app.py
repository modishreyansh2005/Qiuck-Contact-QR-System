...
def register():
    ...
    c.execute(
        "SELECT * FROM qr_codes WHERE code=? AND status='unused'",
        (qr_code,)
    )
    qr = c.fetchone()
    ...
    c.execute(
        "UPDATE qr_codes SET status='assigned' WHERE code=?",
        (qr_code,)
    )
...
def export_csv():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, vehicle_number, owner_name, phone, qr_code FROM vehicles"
    )
    ...
    writer.writerow(
        ["ID", "Vehicle Number", "Owner Name", "Phone", "QR Code"]
    )
